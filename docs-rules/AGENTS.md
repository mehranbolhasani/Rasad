...
Single pipeline entrypoint: `build.py`

Stages: bridges (optional) → `fetcher.py` → `filter.py` → `summarizer.py` → `translator.py` → `grouper.py` → `generator.py` + `feed_output.py` + `text_output.py`

Core models: `rasad/models.py` (`Article`, `SourceRef`, `GroupedStory`).

## Repository Map

- `build.py` — single build CLI (bridges + site)
...
- `templates/` — Jinja2 templates (Farsi, RTL; `base.html`, `index.html`)
- `static/style.css` — minimal stylesheet copied to output
- `deploy_server.sh` — server deploy flow (pull, build, rsync, verify)
